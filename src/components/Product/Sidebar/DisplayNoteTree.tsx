...
const DisplayNoteTree = ({ note, index, selectedNote, setSelectedNote }: DisplayNoteTreeProps) => {
  return (
    <button onClick={() => setSelectedNote(index)} className="flex flex-row items-center w-full hover:bg-interactive-1 active:bg-interactive-2 p-1 text-text-2 hover:text-text-1">
      {selectedNote === index &&
      ...
      >
        {note.data.title.length > (selectedNote === index ? 20 : 22) ? note.data.title.substring(0, (selectedNote === index ? 20 : 22)) + "..." : note.data.title}
      </Text>
      {selectedNote === index && <Text className="text-text-2 ml-auto text-opacity-45">
        {note.data.updatedAt.toLocaleDateString()}
      </Text>}
    </button>
  )
}
